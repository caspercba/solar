...
describe("worker routes", () => {
  ...
  it("GET /api/systems/:id/history/summary returns Growatt vendor daily totals", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-07-03T12:00:00Z"));

    const systems = env();
    const systemId = "sum-1";
    await systems.SYSTEMS.put("_index", JSON.stringify([
      { id: systemId, name: "Summary Site", service: "growatt" },
    ]));
    await systems.SYSTEMS.put(`system:${systemId}`, JSON.stringify({
      id: systemId,
      name: "Summary Site",
      service: "growatt",
    ...
    try {
      const res = await call(
        request(`/api/systems/${systemId}/history/summary?days=7&end=2026-07-03`, { headers: AUTH }),
        systems,
      );

      expect(res.status).toBe(200);
      const json = await res.json();
      expect(json.systemId).toBe(systemId);
