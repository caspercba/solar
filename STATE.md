...
- **SOLAR-0170** — Removing a non-active system from Settings now refreshes the HOME compare grid (`detailRemove` drops the removed id from `compareData` and, when already on HOME, calls `renderComparisonGrid()`/`pollCompareNow()`); no full reload needed. `manage-systems.spec.js` "removing a system refreshes HOME to a single summary tile" passes on desktop-chrome and mobile-chrome.
...
1. **SOLAR-0171** — `backlog`, high (developer) — Fix: duplicate `#admin-create-user-section` in `index.html` (+ duplicate `els` bindings in `app.js`); keep password+confirm form
2. **SOLAR-0175** — `backlog`, high (developer) — Investigate + fix: generator status not shown for Casita del Rio (Growatt off-grid SPF 3500 ES). Shared `isGridActive` / `grid.active` render path looks structurally correct across systems — likely a per-system `gridDetect` threshold misconfig or a live vendor-field quirk on this device; needs a live capture while the generator is running to confirm root cause.
...
- **E2E red on admin create-user** — SOLAR-0171 (duplicate `#admin-create-user-section`); Compare-as-landing acceptance coverage itself passed under SOLAR-0169. SOLAR-0170 (manage-remove) fixed.
